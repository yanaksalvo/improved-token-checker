...
for token in tokens:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            valid_tokens.append((token, user_id, username, discriminator, locale, nsfw_allowed, mfa_enabled, premium_type, verified, phone, email))
            print(f"[{time.strftime('%H:%M:%S')} Valid ({token[:32]}...) - User ID: {user_id}, Username: {username}#{discriminator}, Locale: {locale}, NSFW Allowed: {nsfw_allowed}, MFA Enabled: {mfa_enabled}, Nitro Status: {premium_type}, Verified: {verified}, Phone: {phone}, Email: {email}")
...
with open("valid.txt", "w") as f_valid, open("no_nitro.txt", "w") as f_no_nitro, open("nitro_classic.txt", "w") as f_nitro_classic, open("nitro_boost.txt", "w") as f_nitro_boost, open("verified_true.txt", "w") as f_verified_true, open("verified_false.txt", "w") as f_verified_false, open("phone_true.txt", "w") as f_phone_true, open("phone_false.txt", "w") as f_phone_false, open("email_tokens.txt", "w") as f_email_tokens:
    for token, user_id, username, discriminator, locale, nsfw_allowed, mfa_enabled, premium_type, verified, phone, email in valid_tokens:
        f_valid.write(f"{token.strip()} - User ID: {user_id}, Username: {username}#{discriminator}, Locale: {locale}, NSFW Allowed: {nsfw_allowed}, MFA Enabled: {mfa_enabled}, Nitro Status: {premium_type}, Verified: {verified}, Phone: {phone}, Email: {email}\n")
        if premium_type == 0:
