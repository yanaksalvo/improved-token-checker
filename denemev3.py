...
for token in tokens:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            valid_tokens.append((token, user_id, username, premium_type, verified))
            print(f"[{time.strftime('%H:%M:%S')} Valid ({token[:32]}...) - User ID: {user_id}, Username: {username}, Nitro Status: {premium_type}, Verified: {verified}")
            if premium_type == 0:
...
with open("valid.txt", "w") as f_valid, open("no_nitro.txt", "w") as f_no_nitro, open("nitro_classic.txt", "w") as f_nitro_classic, open("nitro_boost.txt", "w") as f_nitro_boost, open("verified_true.txt", "w") as f_verified_true, open("verified_false.txt", "w") as f_verified_false:
    for token, user_id, username, premium_type, verified in valid_tokens:
        f_valid.write(f"{token.strip()} - User ID: {user_id}, Username: {username}, Nitro Status: {premium_type}, Verified: {verified}\n")
        if premium_type == 0:
