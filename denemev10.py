...
for token in tokens:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            valid_tokens.append((token, user_id, username, discriminator, banner_url, avatar_url, locale, nsfw_allowed, mfa_enabled, creation_date, premium_type, verified, phone, email))
            print(f"[{time.strftime('%H:%M:%S')} Valid ({token[:32]}...) - User ID: {user_id}, Username: {username}#{discriminator}, Banner URL: {banner_url}, Avatar URL: {avatar_url}, Locale: {locale}, NSFW Allowed: {nsfw_allowed}, MFA Enabled: {mfa_enabled}, Creation Date: {creation_date}, Nitro Status: {premium_type}, Verified: {verified}, Phone: {phone}, Email: {email}")
